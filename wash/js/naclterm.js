...
NaClTerm.prototype.run = function() {
  ...
  // Instantly resize the terminal to the same size so we trigger an update.
  this.processManager.onTerminalResize(
    window.tw_.term.screenSize.width,
    window.tw_.term.screenSize.height);
  if (!this.started) {
